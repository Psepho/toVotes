...
toPollGeo$ward <- as.integer(str_sub(toPollGeo$id,1,2))
toPollGeo$area <- as.integer(str_sub(toPollGeo$id,-3,-1))
...
toPollGeo <- inner_join(toPollGeo,ward_regions, by=c("ward"))
rm(ward_regions)
